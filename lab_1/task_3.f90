program card_probability
    ...
    print *, "Условная вероятность P(A|B) того, что вторая карта туз, если первая туз: "
    if (ace_count_A > 0) then
        print '(F10.6)', real(ace_count_AB) / real(ace_count_A)
    else
        print *, "Условная вероятность не может быть рассчитана, так как событие A не произошло."
    end if
