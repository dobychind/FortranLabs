program monty_hall  									! Начало программы моделирования задачи Монти Холла методом Монте-Карло
    ...
    print *, "Частота побед при стратегии без смены выбора: ", &
             real(win_no_change) / num_trials  		! Вероятность победы для стратегии "не менять выбор"
    print *, "Побед при стратегии со сменой выбора: ", win_change  						! Сколько раз победила стратегия "сменить выбор"
    print *, "Частота побед при стратегии со сменой выбора: ", &
             real(win_change) / num_trials  			! Вероятность победы для стратегии "сменить выбор"
