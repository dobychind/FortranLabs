program card_probability
    ...
    ! Выводим результаты
    print '(A, F10.6)', "Безусловная вероятность P(A) того, что вторая карта туз: " // &
        real(ace_count_A) / real(trials)
    ! Выводим безусловную вероятность того, что вторая карта — туз (частота события A)

    print '(A, F10.6)', "Условная вероятность P(A|B) того, что вторая карта туз, если первая туз: " // &
        real(ace_count_AB) / real(ace_count_A)
